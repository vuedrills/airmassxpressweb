...
import { EQUIPMENT_CATEGORIES, EQUIPMENT_CAPACITY_PRESETS } from '@/lib/constants';
...
export default function PostEquipmentPage() {
    ...
    // Get capacities for current category with fallback presets
    const categoryCapacities =
        capacitiesData?.grouped?.[currentTaskDraft.category || ''] ||
        EQUIPMENT_CAPACITY_PRESETS[currentTaskDraft.category || ''] ||
        [];
    ...
    const handleCreateTask = async () => {
        try {
            ...
            // Upload attachments (if any) to Supabase Storage
                            ...
                            <div className="bg-white rounded-lg border p-8 shadow-sm">
                                ...
                                {step === 3 && (
                                    <div className="space-y-6">
                                                    ...
                                                    >
                                                        <input
                                                            ...
                                                            onChange={() => updateTaskDraft({
                                                                hireDurationType: type,
                                                                estimatedHours: undefined, // Reset others
                                        ...
                                        {/* V2: Estimated Duration (if NOT hourly) */}
                                        {['daily', 'weekly', 'monthly'].includes((currentTaskDraft as any).hireDurationType) && (
                                            <div>
                                                <label className="block text-sm font-medium mb-2">
                                                    Duration ({(currentTaskDraft as any).hireDurationType === 'daily' ? 'Days' : (currentTaskDraft as any).hireDurationType === 'weekly' ? 'Weeks' : 'Months'})
                                                </label>
                                                <Input
                                                    type="number"
                                                    value={(currentTaskDraft as any).estimatedDuration || ''}
                                                    onChange={(e) => updateTaskDraft({ estimatedDuration: parseInt(e.target.value) } as any)}
                                                    placeholder={`e.g., 3`}
                                                    min="1"
                                                />
                                            </div>
                                        ...
                                        {/* V2: Fuel Included (De-emphasized) */}
                                        <div className="pt-2">
                                            <label className="flex items-center gap-3 cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    className="w-4 h-4 rounded border-gray-300 text-[#1a2847] focus:ring-[#1a2847]"
                                                    checked={(currentTaskDraft as any).fuelIncluded || false}
                                                    onChange={(e) => updateTaskDraft({ fuelIncluded: e.target.checked } as any)}
                                                />
                                            ...
                                            {(currentTaskDraft as any).hireDurationType && (
                                                <div className="grid grid-cols-3 gap-4 border-b pb-4">
                                                    ...
                                                    <span className="col-span-2 font-medium capitalize">
                                                        ...
                                                        {['daily', 'weekly', 'monthly'].includes((currentTaskDraft as any).hireDurationType) && (currentTaskDraft as any).estimatedDuration && (
                                                            <span className="text-gray-500 ml-1">
                                                                ({(currentTaskDraft as any).estimatedDuration} {(currentTaskDraft as any).hireDurationType === 'daily' ? 'Days' : (currentTaskDraft as any).hireDurationType === 'weekly' ? 'Weeks' : 'Months'})
                                                            </span>
                                                        )}
                                                    </span>
                                                </div>
                                            )}
                                            {/* V2: Fuel Included Review */}
                                            <div className="grid grid-cols-3 gap-4 border-b pb-4">
                                                <span className="text-gray-500">Fuel</span>
                                                <span className="col-span-2 font-medium">
                                                    {(currentTaskDraft as any).fuelIncluded ? '✅ Included' : '❌ Not Included'}
                                                </span>
                                            </div>
                                            {/* V2: Operator Preference */}
                                ...
                                <div className="flex justify-between mt-8 pt-6 border-t">
                                    ...
                                    <div className="ml-auto">
                                        {step < totalSteps ? (
                                            step !== 2 && <Button onClick={nextStep}
                                                disabled={
